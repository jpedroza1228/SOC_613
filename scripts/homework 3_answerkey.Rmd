...
sub %>% 
  ggplot(aes(fct_reorder(as.factor(schoolid), null_diff), null_diff)) +
  geom_errorbar(aes(ymin = null_diff + qnorm(0.025)*null_se,
                    ymax = null_diff + qnorm(0.975)*null_se)) +
...
sub %>% 
  ggplot(aes(fct_reorder(as.factor(schoolid), ran_int_diff), ran_int_diff)) +
  geom_errorbar(aes(ymin = ran_int_diff + qnorm(0.025)*ran_int_se,
                    ymax = ran_int_diff + qnorm(0.975)*ran_int_se)) +
  geom_point(aes(color = schoolid)) +
  theme_minimal() +
  coord_flip()
...
sub %>% 
  ggplot(aes(fct_reorder(as.factor(schoolid), cv_diff), cv_diff)) +
  geom_errorbar(aes(ymin = cv_diff + qnorm(0.025)*cv_se,
                    ymax = cv_diff + qnorm(0.975)*cv_se)) +
  geom_point(aes(color = schoolid)) +
  theme_minimal() +
  facet_wrap(~cv_term) +
  coord_flip()
